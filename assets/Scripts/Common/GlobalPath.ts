
/**
 * 全局路径
 */
export class GlobalPath 
{
    public static SERVER_PATH:string='http://192.168.1.125:8081/';
    /**
     * UI界面预制体目录
     */
    public static UI_PANEL_DIR:string='prefabs/ui_panel/';
    public static ITEM_PREFAB_DIR:string='prefabs/commonItem/';
    public static TREASURE_ITEM_DIR:string='prefabs/TreasureItems/';
    /**
     * resource里的Stage_Property文件夹路径
     */
    public static STAGE_PROPERTY_Path:string='UI/Stage_Property/';

    public static ROLE_ATTRIBUTE_ICON_PATH:string='UI/Stage_Property/Attribute/';
    /** 详细面板文件夹路径 */
    public static DETAIL_PANEL_PATH:string='prefabs/Details/';
    public static CONFIG_PATH:string='datas/Config';
    /**
     * 图集资源路径 
     */
    public static UI_ATLAS_PATH:string='UI/atlas_2';
}
