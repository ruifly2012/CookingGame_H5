...
import { NetMakeCookingInfo, NetMakeCookingNotify, CookingRewardNotify, NetCookingReward } from "../../../NetWork/NetMessage/NetMakeCookingInfo";
...
export class CookingProxy extends Proxy
{
            ...
            if (this.secondTime <= 0)
            {
                ...
                HttpRequest.getInstance().requestPost(RequestType.cook_info, function (info: NetMakeCookingNotify)
                {
                    if (info.remainTime <= 1)
                    {
                        ...
                        if(info.visitorReawrd!=null) self.currVisitors = info.visitorReawrd;
                        self.cookingEnd();
                    }
                    else
                    {
        ...
        HttpRequest.getInstance().requestPost(RequestType.cook_quicken, function (result: VerificationResult)
        {
            if (result == VerificationResult.Success)
            {
                HttpRequest.getInstance().requestPost(RequestType.cook_info, function (e: NetMakeCookingNotify)
                {
                    ...
                    if(e.visitorReawrd!=null) self.currVisitors =e.visitorReawrd;
                    self.cookingEnd();
                });
            }
            else
    ...
    cookingEnd()
    {
        ...
        Log.Info('--------------------做菜完成--------------------');
        HttpRequest.getInstance().requestPost(RequestType.props_info,function(e){
            this.menuProxy.configCookMenu();
        }.bind(this));
        ...
        {
            if (cookingVo == null || cookingVo == undefined) return;
            cookingVo.role._NowState=FigureStatus.Leisure;
            cookingVo.role._CurrMission=FigureStatus.Leisure;
    ...
    */
    ...
    showVisitor()
    {
        ...
        let rewards:NetCookingReward[]=[];
        for (let i = 0; i < eval(this.currVisitors).length; i++) {
            console.dir(eval(this.currVisitors)[i]);
            rewards.push(Object.assign(new NetCookingReward(),eval(this.currVisitors)[i]));
        }
        ...
        for (let i = 0; i < rewards.length; i++)
        {
            ...
            let visitor: VisitorDataBase = DataManager.getInstance().VisitorMap.get(Number(element.visitorId));
            visitor._ID = Number(element.visitorId);
            visitor._RuneID = Number(element.rewards[0]);
            visitors.push(visitor);
            GameStorage.setItem(element.visitorId.toString(), 1);
