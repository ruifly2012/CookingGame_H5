...
export class ServerSimulator
{
    ...
    private filterHadNum(_conditionKeys: Array<any>)
    {
        for (let i = 0; i < _conditionKeys.length; i++)
        {
            if (_conditionKeys[i] == 1)
            {
                this.filterVal = DataManager.getInstance().TableRoleMap.size;
                this.currMission._CompleteVal = this.currMission._Condition.get(_conditionKeys[i]);
            }
            else if (_conditionKeys[i] == 2)
            {
                this.filterVal = DataManager.getInstance().TableMenuMap.size;
                this.currMission._CompleteVal = this.currMission._Condition.get(_conditionKeys[i]);
            }
    ...
    getServerCurrMission(): Mission
    {
        ...
        {
            GameStorage.setItemJson(ServerSimulator.MISSION_DATA, DataManager.getInstance().MissionMap.get(10020));
            let _mission: Mission = DataManager.getInstance().MissionMap.get(10020);
            return _mission;
        }
    }
