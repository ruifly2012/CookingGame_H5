...
export default class OnhookPanelView extends cc.Component {
    ...
    OnClickEvent(btn: any) {
        switch (btn.node.name) {
            ...
            break
            case'LevelUp':
            var hb = DataManager.getInstance().OnhookMap.get(DataManager.getInstance().OnhookMap.has(this.Onhooks._ID+1)?this.Onhooks._ID+1:this.Onhooks._ID);//获取当前id的数据
            if(DataManager.getInstance().getLevelData((hb._UnlockID).toString()) == null){
                Log.ShowLog('通关'+DataManager.getInstance().OnhookMap.get(hb._UnlockID)._Name+'关卡解锁');
