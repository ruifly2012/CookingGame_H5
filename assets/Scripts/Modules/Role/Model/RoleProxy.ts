...
export class RoleProxy extends Proxy implements IProxy {
    ...
    netInit(info:NetRoleInfo[])
    {
        for (let i = 0; i < info.length; i++) {
            ...
            let role:PresonDataBase=DataManager.getInstance().baseRoleMap.get(info[i].id);
            role._Level=_info.level;
            role._AdvanceLevel=_info.advanceLevel;
            role._Power=_info.power;
            role._Agility=_info.agility;
            role._PhysicalPower=_info.physicalPower;
            role._Will=_info.will;
            role._Cooking=_info.cooking;
            role._Vigor=_info.vigor;
            role._Savvy=_info.savvy;
            role._Luck=_info.luck;
            role._NowState=_info.characterState;
        }
    }
    ...
    takeoutAdvanceProp(id:number)
    {
        ...
        DataManager.getInstance().PropVoMap.get(vo._PropID)._Amount-=vo._PropNum;
        DataManager.getInstance().PropVoMap.get(vo._PropID)._Amount=MathTool.Abs(DataManager.getInstance().PropVoMap.get(vo._PropID)._Amount);
        DataManager.getInstance().savePropNum(vo._PropID, DataManager.getInstance().PropVoMap.get(vo._PropID)._Amount);
    }
